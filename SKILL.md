...
If `inspect` reports `insufficient_low_risk_inventory`:

- Stop at `inspect`.
- Report the current low-risk inventory count, risk-excluded outputs, and `recommendedNextStep`.
- Do not automatically relax `--risk-policy conservative`.
- Do not process risk-excluded outputs automatically.
- Do not enter `prepare`, `guarded`, or `execute`.
- Recommend generating new low-risk content candidates, then wait for user topic confirmation.
...
- If `readyForPrepare` and `readyForGuardedDryRun` are both below the requested count and all remaining unpublished outputs are excluded by risk policy, `inspect` reports `inventoryStatus = insufficient_low_risk_inventory`, `availableReadyForPrepare`, `availableReadyForGuardedDryRun`, `riskExcludedCount`, and `recommendedNextStep = codex_generate_low_risk_topic_candidates`.
